...
Handle<Value> Image::WriteFile(const Arguments& args) { return generic_start<Image>(eWriteFile, args, kWriteFile); }
...
Handle<Value> Image::Write(const Arguments& args) {
  HandleScope scope;
  int aOpt[1];
  if (checkArguments(kWrite1, args, aOpt))
...
void Image::Generic_process(void* pData, void* pThat) {
  ...
  case eWriteFile: that->getImage().write(*data->val[0].string); data->retVal.pointer = that;       break;
  case eWrite1:
  case eWrite2:
  case eWrite3: {
...
Handle<Value> Image::Generic_convert(void* pData) {
  ...
  case eNew3: 
  case eWriteFile:
    aResult = ((Image*) data->retVal.pointer)->handle_; break;
  case eWrite1:
  case eWrite2:
  case eWrite3: {
